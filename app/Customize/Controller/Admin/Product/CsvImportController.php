...
class CsvImportController extends AbstractCsvImportController
{
                ...
                if (!empty($formFile)) {
                    ...
                    foreach ($data as $row) {
                        ...
                        if ($ProductClasses->count() < 1) {
                            ...
                            // 税区分 0:税抜 1:税込 未指定の場合0
                            if (StringUtil::isBlank($row[$headerByKey['zei_kubun']])) {
                                $ProductClassOrg->setZeiKubun(0);
                            } else {
                                if(preg_match('/^[01]$/', $row[$headerByKey['zei_kubun']])) {
                                    $zei_kubun = $row[$headerByKey['zei_kubun']];
                                    $ProductClassOrg->setZeiKubun($zei_kubun);
                                } else {
                                    $message = trans('admin.common.csv_invalid_not_found', ['%line%' => $line, '%name%' => $headerByKey['zei_kubun']]);
                                    $this->addErrors($message);
                                }
                            }
                            ...
                            if (!$flag) {
                                ...
                                if (isset($row[$headerByKey['class_category1']]) && isset($row[$headerByKey['class_category2']])
                                    ...
                                    // 税区分 0:税抜 1:税込 未指定の場合0
                                    if (StringUtil::isBlank($row[$headerByKey['zei_kubun']])) {
                                        $ProductClass->setZeiKubun(0);
                                    } else {
                                        if(preg_match('/^[01]$/', $row[$headerByKey['zei_kubun']])) {
                                            $zei_kubun = $row[$headerByKey['zei_kubun']];
                                            $ProductClass->setZeiKubun($zei_kubun);
                                        } else {
                                            $message = trans('admin.common.csv_invalid_not_found', ['%line%' => $line, '%name%' => $headerByKey['zei_kubun']]);
                                            $this->addErrors($message);
                                        }
    ...
    protected function getProductCsvHeader()
    {
        return [
            ...
            trans('admin.product.product_csv.zei_kubun_col') => [
                'id' => 'zei_kubun',
                'description' => 'admin.product.product_csv.zei_kubun_description',
                'required' => false,
            ],
        ];
    }
